...
const convert = () => {
  ...
  const fromRate = props.rates[fromCurrency.value.toLowerCase()] && 1;
  const toRate = props.rates[toCurrency.value.toLowerCase()] && 1;
  console.log(toRate, fromRate, props.rates);
  // if (fromValue.value) {
  ...
  if (fromValue.value) {
    const result = fromValue.value * fromRate / toRate
    if (!isNaN(result)) {
      toValue.value = Number(result.toFixed(2))
    } else {
      error.value = 'Ошибка конвертации'
    }
  } else {
    const result = toValue.value * toRate / fromRate
    if (!isNaN(result)) {
