...
const collectCurrencies = () => {
  ...
  for (const pair in props.rates) {
    ...
    if (!arrCurrencies.value[from]) {
      arrCurrencies.value[from] = {}
    }
    arrCurrencies.value[from][to] = props.rates[pair]
  }
  // Добавляем RUB в список валют
  if (!arrCurrencies.value['RUB']) {
    arrCurrencies.value['RUB'] = { RUB: 1 }
  }
}
